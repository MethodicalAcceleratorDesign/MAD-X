...
!---- Get transfer matrix.
        call tmpart(orbit0,.true.,.true.,opt_fun0,rt,tt,eflag)
...
!---- Test for overflow.
 ...
 999  end
      ...
      subroutine tmpart(guess,fsec,ftrk,opt_fun0,rt,tt,eflag)
...
!----------------------------------------------------------------------*
      ...
      integer eflag,i,k,irank,itra,itmax,get_option,save_opt,thr_on
      parameter(itmax=20)
      double precision guess(6),opt_fun0(*),rt(6,6),tt(6,6,6),cotol,    &
     &orbit0(6),orbit(6),a(6,7),b(4,5),as(3,4),bs(2,3),deltap,get_value,&
     &zero,one,get_variable
      parameter(zero=0d0,one=1d0)
      equivalence(a(1,1),b(1,1),as(1,1),bs(1,1))
...
!----------------------------------------------------------------------*
     ...
     &node_al_errors,n_align
      double precision rt(6,6),ek(6),re(6,6),rm(6,6),rwi(6,6),rc(6,6),  &
     &te(6,6,6),el,elh,orbit(6),orbit0(6),betas,gammas,                 &
...
!----------------------------------------------------------------------*
     ...
     &node_al_errors
      double precision orbit(6),orbit0(6),ek(6),re(6,6),rm(6,6),        &
     &te(6,6,6),tm(6,6,6),al_errors(align_max),deltap,el,betas,gammas,  &
     &node_value,get_value,pos0,elh,zero,one,two
      parameter(zero=0d0,one=1d0,two=2d0)
      character*130 msg
...
!---- Physical element.
      ...
      if(centre) then
        pos0=currpos
        currpos=currpos+el/two
        if(save.ne.0) call twfill(2,opt_fun,currpos, 1)
      endif
      centre_bttk=.false.
      if (fmap) then
        call twbttk(re,te)
...
!---- centre option
      ...
      integer get_option
      double precision parvec(26),orbit(6),re(6,6),te(6,6,6),pi,r0x,r0y,&
     &xm,ym,r0x2,r0y2,xs,ys,rho2,fk,phix,phiy,phixx,phixy,phiyy,phixxx, &
     &phixxy,phixyy,phiyyy,get_variable,get_value,node_value,zero,one,  &
