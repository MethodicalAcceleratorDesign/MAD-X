...
int main()
{
  ...
  if ((mangler = NameMangler_init())) {
    ...
    for (int i=0; i<20*20; i++) {
      ...
      std::cout << i << '\t' << NameMangler_mangle(mangler, (std::string("PIFF") + char('a' + (i%20)) + char((i/20 + 'A'))).c_str(), buf) << std::endl;
    }
  
    while(std::cin) {
      std::string str;
      if (std::cin>>str) {
	std::cout << NameMangler_demangle(mangler, str.c_str(), buf) << std::endl;
      } else {
	break;
      }
    }
