...
void pro_distribution(struct in_cmd* p){
	...
	double x[npart], px[npart], y[npart], py[npart], t[npart], pt[npart];
	...
	getunconvertedcoord(x,px,y,py,t,pt, &npart);
	...
	for(int i=0; i< npart; i++){
		double_to_table_curr(table_name, "x", &x[i]);
	 	double_to_table_curr(table_name, "px", &px[i]);
	 	double_to_table_curr(table_name, "y", &y[i]);
	 	double_to_table_curr(table_name, "py", &py[i]);
	 	double_to_table_curr(table_name, "t", &t[i]);
	 	double_to_table_curr(table_name, "pt", &pt[i]);
	 	num = (int)i;
