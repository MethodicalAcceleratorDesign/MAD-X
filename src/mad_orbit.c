...
static int     pro_correct_getactive(int ip, int *nm, int *nx, int *nc, double *corvec, double *monvec,char *conm);
static void    pro_correct_write_results(double *monvec, double *resvec, double *corvec, int *nx, int *nc, int *nm, int imon, int icor, int ip);
...
static int     pro_correct2_getactive(int ip, int *nm, int *nx, int *nc, double *corvec, double *monvec,char *conm);
...
static void    pro_correct2_write_results(double *monvec, double *resvec, double *corvec, int *nx, int *nc, int *nm, int imon, int icor, int ip);
...
static void
fill_orbit_table(struct table* t_out, struct table* t_in)
...
static void
correct_setcorr(struct in_cmd* cmd)
{
...
static void
correct_readcorr(struct in_cmd* cmd)
{
...
static int
pro_correct2_getorbit(struct in_cmd* cmd)
{
  struct name_list* nl;
  int i;
  int pos;
...
static int
pro_correct2_getcorrs(struct in_cmd* cmd)
{
  int debug;
...
static int
pro_correct2_getactive(int ip, int *nm, int *nx, int *nc, double *corvec, double *monvec,char *conm)
{
  int  imon, icor;
  int  imona, icora;
  struct id_mic2 *m, *c;
...
static double*
pro_correct2_response_ring(int ip, int nc, int nm)
{
  int    ic, im;
...
/* initialize imat: */
  ...
  while(c) {
    if (debug) printf("corrector flag: %d\n",c->enable);
	...
	im = 0;
	while(m) {
	  if (debug) printf("monitor flag: %d\n",m->enable);
...
static void
pro_correct2_write_results(double *monvec, double *resvec, double *corvec, int *nx, int *nc, int *nm, int imon, int icor, int ip)
{
/*                                              */
...
/* Steering routine for orbit corrections of one beam */
{
  ...
  if(strcmp("micado",command_par_string("mode",cmd->clone)) == 0) {
    ...
    if (ifail == 0) pro_correct_write_results(monvec, resvec, corvec, nx, nc, nm, imon, icor, ip);
    else
      {
       printf("MICADO correction completed with error code %d\n\n",ifail);
       warning("MICADO back with error",", no correction done");
...
static void
correct_correct(struct in_cmd* cmd)
...
/* Get access to tables, for orbit and model the default is twiss_table */
  ...
  if ((orbtab = command_par_string("orbit",cmd->clone)) != NULL) {
    ...
    if((orbin_table = twiss_table) == NULL) {
      ...
      if (debug) printf("orbit from TWISS table at address: %p\n", (void*)twiss_table);
    }
  ...
  if ((tartab = command_par_string("target",cmd->clone)) != NULL) {
    ...
    if (debug) printf("No target orbit requested\n");
  }
  // 2013-Jun-24  12:00:18  ghislain: commented out; never used.
  // ppt = -1;
  ...
  if ((modtab = command_par_string("model",cmd->clone)) != NULL) {
    ...
    if((model_table = twiss_table) == NULL) {
      ...
      if (debug) printf("model from TWISS table at address: %p\n", (void*)twiss_table);
    }
    // 2013-Jun-24  12:00:18  ghislain: commented out; never used.
    // ppt = -1;
  }

  if (debug) printf("The orbit, twiss, target and model tables are at addresses: %p %p %p %p\n",
...
static int
pro_correct_getorbit(struct in_cmd* cmd)
{
  struct name_list* nl;
  int i;
...
static int
pro_correct_getorbit_ext(struct in_cmd* cmd)
{
  struct name_list* nl;
  int i;
  int j;
  ...
  while(m) {
    ...
    if (debug) printf("monitor name: %s\n",l2name);
	...
	if(strncmp(l4name,l2name,strlen(l2name)) == 0) {
	  ...
	  jjy = jj-1;
	  yok = 1;
	  if(debug) printf("monitor names found: %s %s %d\n",l2name,l4name,yok);
	}
      }
    }
    if(debug) printf("jjx,jjy %d %d\n",jjx,jjy);
...
static int
pro_correct_getcorrs(struct in_cmd* cmd)
{
  int debug;
...
static void
pro_correct_write_cocu_table(void)
{
  int i,j;
  int pr_cols;
...
static void
pro_correct_make_corr_table(void)
{
  struct table *ttb;
  int j;
...
static void
pro_correct2_make_corr_table(void)
{
  struct id_mic2 *ttb;
...
static void
pro_correct_make_mon_table(void)
{
  struct table *ttb;
  int j;
...
static void
pro_correct2_make_mon_table(void)
{
  struct id_mic2 *ttb;
/*
  static char atm[3][4] = {"hmon","vmon","moni"};
...
static void
pro_correct_fill_corr_table(int ip ,char *name, double old, double new)
{
  struct table *cor;
...
static void
pro_correct2_fill_corr_table(int b, int ip ,char *name, double old, double new)
{
...
static void
pro_correct_fill_mon_table(int ip ,char *name, double old, double new)
{
  struct table *mon;
...
static void
pro_correct2_fill_mon_table(int ip ,char *name, double old, double new)
{
  struct table *mon;
...
static void
pro_correct_write_results(double *monvec, double *resvec, double *corvec, int *nx, int *nc, int *nm, int imon, int icor, int ip)
/*                                              */
/* Writes a summary of the correction           */
/* Writes correctors strengths into sequences   */
/* Fills TFS tables for correctors and monitors */
...
static int
pro_correct_getactive(int ip, int *nm, int *nx, int *nc, double *corvec, double *monvec,char *conm)
{
  int  imon, icor;
  int  imona, icora;
  struct id_mic *m, *c;
...
static void
correct_option(struct in_cmd* cmd)
{
  struct name_list* nl = cmd->clone->par_names;
  int i, debug;
  int pos, seed;
  ...
  if (debug) {
    ...
    else fprintf(prt_file, "print option set\n");
  }
  ...
  if (debug) {
    ...
    else fprintf(prt_file, "debug option set\n");
  }
...
static void
correct_putorbit(struct in_cmd* cmd)
{
  int i;
  struct name_list* nl;
...
static void
correct_usekick(struct in_cmd* cmd)
{
  char temp[12];
...
static void
correct_usemonitor(struct in_cmd* cmd)
{
  char temp[12];
...
void
store_orbit(struct command* comm, double* orbit)
{
...
void
f_ctof(int *j, char *string, int *nel)
{
  long i, flg = 0;
