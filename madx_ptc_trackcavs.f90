...
use madx_ptc_setcavs_module
  ...
  integer, private                    :: skowrondebug  = 9     ! flag for debugging ranges from 0 (no debug printout) to 10 (the most detailed)
        ...
        if (.true.) then
         ...
         !          print *, "opt_fun(ii)",opt_fun(ii)
                ...
                if ( (associated(p%next) .eqv. .false. ) .or. (associated( p%next, my_ring%start)) ) then
                  ...
                  nfen=p                                          !if it is the last element in the line 
                  print *, 'It is the last element  ', p%mag%name  !(it is always marker, i.e element that does not change reference energy) 
                  print *, 'Its reference energy is ', nfen%p0c  !take its reference energy
                else
