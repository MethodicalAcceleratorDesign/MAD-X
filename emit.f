...
*---- Undamped tunes and beam extents.
      ...
      if (qs .lt. zero) qs = - qs
*---- Summary output.
      call emsumm('EMIT', rd, em, bmax, gmax, dismax, stabt, frad,      &
...
*----------------------------------------------------------------------*
      include 'emit.fi'
      integer j,j1,j2,k,k1,k2,nline,iqpr2
      ...
      double precision dismax(4),get_variable,get_value
      double precision emit_v(3), nemit_v(3), tunes(3), sig_v(4)
      character*(*) comand
      logical stabt, frad
...
*---------------------------------------------------------------------*
      ...
      double precision  an2, x1, y1, t1, x2, y2, t2, px1, py1, pt1
      ...
      double precision  el, node_value, sk0, sk0s, tilt, bv0, bvk
      ...
      double precision  sk3, rfac, dbr, dbi, dipr, dipi, rfacx, rfacy
...
*---- Prepare data.
        ...
        sk0 = bv0 * sk0
        sk0s = bv0 * sk0s
        edg1 = bv0 * node_value('e1 ')
        edg2 = bv0 * node_value('e2 ')
