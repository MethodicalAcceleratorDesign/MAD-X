...
\begin{madlist}
   \ttitem{CALLS} The maximum number of calls to the penalty function. (Default: 1000)
   \ttitem{TOLERANCE} The desired tolerance for the minimum. (Default: 1E-6) 
\end{madlist}
...
\begin{madlist}
  \ttitem{CALLS} The maximum number of calls to the penalty function. (Default: 30) 

  \ttitem{TOLERANCE} The desired tolerance for the minimum. (Default: 1E-6)

  \ttitem{REPEAT} The number of call of the JACOBIAN routine. (Default: 1) 
  ...
  \ttitem{COOL, BALANCE} The factors which specify the following transformation:
\begin{verbatim}
  if "balance" >=0
