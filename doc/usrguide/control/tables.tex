...
%% last row, or \texttt{ROW - 1 $<$ -nrow}, ie trying to access the table
%% backwards before the first row, results in a ``row out of bound''
%% message and the expression is not constructed or evaluated. 

%% %% to be implemented
%% Note that the special value \texttt{ROW~=~0} denotes the last row of
%% the table. \\
%% Consequently \texttt{ROW~=~-1} denotes the one before last
...
\madbox{
SETVARS\_LINE, TABLE=tablename, ROW1=integer, ROW2=integer, PARAM=real;
}
sets the variables with values calculated by linear interpolation
between the values in ROW1 and ROW2 of table "tablename". The string
given in the PARAM attribute is the interpolation factor. For proper
interpolation, \texttt{PARAM} should be between 0 and 1; \texttt{PARAM}
below 0 or above 1 will perform extrapolation accordingly. 
...
Internally, \madx constructs this expression as a deferred expression
and evaluates it. 
...
WHILE \=(i < len) \{ \\
      \>i = i + 1; \\
      \>SETVARS\_LIN, TABLE=twiss, ROW1=i, ROW2=i+1, PARAM=0.5; \\
      \>! now variables are interpolated at the center of the elements. \\
      \>! In particular the S variable is the position \\
      \>! of the center of the element  \\
      \>VALUE, S; \\
      \>\ldots \\ \};
}

Note that the special value \texttt{ROWi~=~0} denotes the last row of
the table. \\
