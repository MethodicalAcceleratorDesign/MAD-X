...
of freedom. It works on the coupled lattice functions defined in
...
the optics parameters w.r.t. deltap/p. If one is interested in
evaluating deltap/p-dependency of the Twiss parameters, one must ensure
that the order (no) of the map is set to 2 at least. 
The derivatives of the dispersion w.r.t. deltap/p have column
names:  
...
PTC_TWISS, icase=integer, deltap=double, closed_orbit, slice_magnets, 
           ...
           initial_map_manual, beta0=string,   
...
Note that the value is internally set to 56 when attempting to set
icase=6 with no cavity. 

\item {\bf NO}=integer (Default: 1) \\
the order of the map, which is often supplied as 1 but must be at
least set to 2 to retrieve chromaticities for instance. For evaluating
the Twiss parameters derivatives w.r.t. deltap/p, one must pass order 2
at least. 
...
\item {\bf CENTER\_MAGNETS}=logical (Default: .false.) \\
the switch to turn on the evaluation of Twiss
parameters at the middle of each magnet. This relies on internal slicing
and 'integration nodes' as determined by the number of steps (nst)
selected when creating the PTC layout. This number is assumed to be even
...
\item {\bf FILE}[=string] (Default: ptc\_twiss) \\
if FILE is omitted, no output is written to file. \\
If FILE is present, the optional argument is the name of the file for
printing the PTC\_TWISS output. 

\item {\bf TABLE}[=string] (Default: ptc\_twiss) \\
if TABLE is omitted, no output is written to an internal table. \\
If TABLE is present, the optional argument is the name of the internal
PTC\_TWISS table. 

\item {\bf SUMMARY\_FILE}[=string] (Default: ptc\_twiss\_summary) \\
if SUMMARY\_FILE is omitted, no output is written to file. \\ 
If SUMMARY\_FILE is present, the optional argument is the name of the file
for printing the PTC\_TWISS\_SUMMARY table output.

\item {\bf SUMMARY\_TABLE}[=string] (Default: ptc\_twiss\_summary) \\
if SUMMARY\_TABLE is omitted, no output is written to an internal
table. \\
If SUMMARY\_TABLE is present, the optional argument is the name
of the internal PTC\_TWISS\_SUMMARY table.  

\item \href{../Introduction/ranges.html#range}{\bf RANGE}=string (Default:
  \#s/\#e) \\ 
specifies a segment of beam-line for the PTC\_TWISS calculation.

\item {\bf INITIAL\_MATRIX\_TABLE}=logical (Default: .false.) \\
Reading  the transfer matrix from the map-table in memory (a preceding
\texttt{\href{../ptc_normal/ptc_normal.html}{PTC\_NORMAL}} command is
needed). 

\item {\bf INITIAL\_MATRIX\_MANUAL}=logical (Default: .false.) \\ 
Using the input variables RE11,...,RE66 (see next entry) as the transfer
matrix.   

\item {\bf INITIAL\_MAP\_MANUAL}=logical (Default: .false.) \\
...
\item {\bf RE11,..., RE66} = doubles (Default: Unit Matrix) \\ 
Values of the \(6\times 6\) transfer matrix.  

\item {\bf BETA0}=string (Default: beta0) \\
...
\item {\bf betx, alfx, mux, bety, alfy, muy, dx, dpx, dy, dpy} = doubles
  (Default: 0) \\
