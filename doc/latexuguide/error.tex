...
WARNING: if errors are to be applied to a thin sequence produced by \hyperref[chap:makethin]{\texttt{MKTHIN}}, it is advisable to save the sequence on a file and then reload it, in order for \madx to reinitialize its data structures in a suitable way.
...
\begin{madlist}
  ...
  123456789). \texttt{SEED} alone continues with the current sequence See
  also: \hyperref[subsubsec:random]{Random Values}. \texttt{SEED} may be an expression.  
