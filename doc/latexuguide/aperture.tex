...
\begin{madlist}
  ...
  preselected types:
    \begin{madlist}
      ...
      \ttitem{RECTANGLE} 
      \ttitem{ELLIPSE} 
      \ttitem{RECTCIRCLE} a superposition of a {\tt CIRCLE} and
        a {\tt RECTANGLE} 
      \ttitem{LHCSCREEN} an alias for {\tt RECTCIRCLE}
      \ttitem{RECTELLIPSE} a superposition of an {\tt ELLIPSE} and a
        {\tt RECTANGLE}
      \ttitem{RACETRACK} a circle separated in four
      quartercircles, one per quadrant, and connected by straight lines.
      \begin{5.02.05}
      \ttitem{OCTAGON} a simply connex octagon\note[GR]{This is an addition in 
      5.02.05 per request \# 324 in TRAC}
      \end{5.02.05}
    \end{madlist}
    or from a file if a filename is provided as argument. The file contains a 
    list of x and y coordinates outlining the complete aperture shape and no 
    symmetry is assumed. This option is only 
    supported by the aperture module, see below. In this case the {\tt 
    APERTURE} attribute is ignored.\\
\\
...
\hline
{\tt ELLIPSE} & 2 & horizontal an vertical semi-axes of ellipse \\  
\hline
...
%\hline
{\tt RECTELLIPSE} & 4 & half width and half height of rectangle, horizontal
and vertical semi-axes of ellipse \\  
\hline
{\tt RACETRACK} & 3 & horizontal and vertical offsets (g and s) of the circle 
center, radius (r) of the circular part (see figure below)\\  
\hline
\end{tabular}
\end{5.02.04}
...
\begin{tabular}{|l | c | p{9cm}|}
	...
	circle\\  
	\hline
%	MARGUERITE & 3 & half width and half height of rectangle, radius of 
%	circle\\  
%	\hline
	{\tt RECTELLIPSE} & 4 & half width and half height of rectangle, horizontal
	and vertical semi-axes of ellipse \\
	...
	\hline
	{\tt OCTAGON} & 4 & horizontal and vertical extents along axes, two angles 
	sustaining the cut corner. \note[GR]{This is an addition in 5.02.05 per 
	request \# 324 in TRAC}\\
	\hline
	{\tt OCTAGON} (var.) & 4 & horizontal and vertical positions of the two 
	points in the 
	first quadrant. \note[GR]{This is a variant for the addition in 5.02.05 per 
	request \# 324 in TRAC}\\
	\hline
\end{tabular}
\\
...
}\note[GR]{This is in fact a circular aperture; need to check the LHC data}
...
The minimum {\tt n1} for each element is written to the last Twiss table, to
...
given as input using the definition \madxmp{ EX = EXN / GAMMA }
while the \hyperref[chap:beam]{\tt BEAM} command in \madx uses the definition 
\madxmp{ EX = EXN / (4 * BETA * GAMMA) } 
...
The \hyperref[sec:select]{\tt SELECT} command can be  used to
...
\begin{figure}[ht]  
  \begin{center}
    \includegraphics[width=450px]{jpg/aperexample.jpg}
