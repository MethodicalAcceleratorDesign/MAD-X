...
\begin{madlist}
  ...
  $\epsilon_{xn} = 4 \sqrt{\gamma^2 - 1} \ \epsilon_x = 4 \beta \gamma\ \epsilon_x$
  \ttitem{EYN} The normalised vertical emittance [m]:
  $\epsilon_{yn} = 4 \sqrt{\gamma^2 - 1} \ \epsilon_y = 4 \beta \gamma\ \epsilon_y$
...
\note[GR]{the factor 4 in the normalised emittance formula is historical. It 
might be time we move to the standard definition of exn = beta gamma ex. It 
could be that we have to leave the current definition in MAD-X but move to the 
new one for the next MAD...}

\begin{5.02.05}
Note that up to version 5.02.04 the definition of normalised emittance used in 
\madx was referring to the so-called 2-sigma geometric emittance as in:
