...
linked via the interface. With the \hyperref[sec:ptc_create_layout]{\tt
...
next call to \hyperref[sec:ptc_create_layout]{\tt PTC\_CREATE\_LAYOUT}. 
For example, a \hyperref[sec:defer]{deferred expression} of \madx is
only evaluated at the time of the \hyperref[sec:ptc_create_layout]{\tt
...
\hyperref[chap:ptc_track]{\tt PTC\_TRACK},
\hyperref[chap:ptc_track_line]{\tt PTC\_TRACK\_LINE}. 
...
\begin{madlist}
  ...
  \begin{madlist}
    ...
    \ttitem{6D} if {\tt TIME} is true, the \hyperref[sec:canon]{\madx
 ...
 the relative momentum deviation $\delta_p$.
  ...
  \ttitem{NST} the number of integration steps: 1, 2 or 3. \\
...
\begin{madlist}
  ...
  (\hyperref[sec:sbend]{\tt SBEND} or \hyperref[sec:rbend]{\tt RBEND})
  as \hyperref[sec:marker]{\tt MARKER} if their length is equal to zero. 
     
  \ttitem{NST} gives a specific \hyperref[sec:ptc_twiss]{\tt NST}
  ...
  elements: 
  \hyperref[sec:sbend]{\tt sbend}, 
  \hyperref[sec:rbend]{\tt rbend},
  ...
  A full range of additional multipole \hyperref[sec:error_field]{field
