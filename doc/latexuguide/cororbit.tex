...
\begin{madlist}
   ...
   \ttitem{PLANE} If this attribute is {\tt x}, only the horizontal correction
   ...
   \ttitem{COND} When {\tt COND} is 1, a Singular Value Decomposition is
   ...
   \ttitem{NCORR} Only used by the {\tt MICADO} algorithm. Defines the number of
...
\begin{madlist}
   ...
   \ttitem{EXTERN} (default: false): When false, the {\tt ORBIT} and 
     {\tt TARGET} table
   ...
   \ttitem{X\_COL} The name of the column that contains the {\tt X} position 
     readings of monitors in the tables. (Default: "{\tt X}")
   \ttitem{Y\_COL} The name of the column that contains the {\tt Y} position 
     readings of monitors in the tables. (Default: "{\tt Y}")
     
\end{madlist}

Example of use of {\tt CORRECT} to reproduce a measured orbit: 
\begin{verbatim}
...
\begin{madlist}
   ...
   \ttitem{TWISSUM} If the attribute value is larger than 0, {\tt CORRECT}
