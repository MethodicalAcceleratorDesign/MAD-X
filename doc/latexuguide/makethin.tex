...
\begin{5.02.04}
\madbox{
MAKETHIN, \=SEQUENCE=seqname, STYLE=string, \\
          \>MAKEDIPEDGE=logical, \\
...
\\ \end{5.02.04}
   ...
   {\tt DIPEDGE} elements. \\ 
   \change[GR]{(Default:~false)}{(Default:~true)}


\begin{5.02.04}
   \ttitem{MINIMIZEPARENTS} is a logical flag to control the removal of
   inconsistent numbers of slices for parent elements. \\
   (Default:~true)

   \ttitem{MAKECONSISTENT} is a logical flag to ensure an equal number of slices
