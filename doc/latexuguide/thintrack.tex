...
modules. Now, three types of tracking modules (symplectic) are implemented into \madx. 
...
The third option is direct thick tracking using the TRACK module. Since version 5.00.02, % TO BE VERIFIED
thick tracking has been reintroduced in MAD-X. The thick maps of quadrupole and sector-bend have bee implemented in a symplecitc manner.
...
\section{Overview of the Thin- and Thick-Lens Tracking} % Module (thintrack)}
\label{sec:trackoverview}

The \textbf{%thin-lens
	tracking module} of \madx performs element-per-element tracking of one or several particle trajectories in the last
...
Only thin elements are allowed, with some important exceptions: the element \texttt{drift}, and the elements \texttt{quadrupole} and \texttt{sbend} whenever the boolean flag \texttt{THICK} is set to true. The symplecticity of the coordinate transformation is guaranteed in all cases.
