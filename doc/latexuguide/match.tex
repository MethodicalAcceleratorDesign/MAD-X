...
\item Define Constraint
  ...
  \end{itemize}
\item \hyperref[sec:match_methods]{Matching Methods}
  \begin{itemize}
  \item \hyperref[subsec:match_lmdif]{\tt LMDIF}: Fast Gradient Minimisation
  \item \hyperref[subsec:match_migrad]{\tt MIGRAD}: Gradient Minimisation
  \item \hyperref[subsec:match_simplex]{\tt SIMPLEX}: Simplex Minimisation
  \item \hyperref[subsec:match_jacobian]{\tt JACOBIAN}: Newton Minimisation
  \end{itemize}
\item Expression Matching with \hyperref[sec:use_macro]{\tt USE\_MACRO}
\end{itemize}
