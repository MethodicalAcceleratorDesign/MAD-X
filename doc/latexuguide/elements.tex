...
\begin{madlist}
   ...
   \ttitem{LAG} The phase lag [$2\pi$] (DEFAULT: 0). Zero lag means the highest acceleration, 
                ...
                is on the crest of the wave. Please not that it is different from 
...
Alternating Current (AC) dipole (orbit corrector) is implemented with: 
\begin{madlist}
   \ttitem{HACDIPOLE} an AC orbit corrector for the horizontal plane, 
   \ttitem{VACDIPOLE} an AC orbit corrector for the vertical plane, 
\end{madlist}

\madbox{
xxxxxxxxxxxxxxxx\=xxxxxxxxxxxxxxxxxxxx\= \kill
label: HACDIPOLE, L=real, VOLT=real,  FREQ=real, 
               \> RAMP1=integer, RAMP2=integer, RAMP3=integer, RAMP4=integer; \\
label: VACDIPOLE, L=real, VOLT=real,  FREQ=real, 
               \> RAMP1=integer, RAMP2=integer, RAMP3=integer, RAMP4=integer; \\
}
