...
{
  int stop=0;
  int i, nint=1, jslice=1, halo_q_length, halolength, pipelength, namelen=NAME_L, nhalopar, ntol;
  double on_ap=1, on_elem=0;
  double mass, energy, exn, eyn, dqf, betaqfx, dp, dparx, dpary;
  double cor, bbeat, nco, halo[4], interval, spec, ex, ey;
  ...
  char *halofile, *pipefile, *minimum;
  ...
  /* setup of parameters and aperture table */

  /* build halo polygon based on input ratio values */
  ...
  /* end setup of first node start-values */

  /* Initialize n1 limit value */
  lim_pt->n1=999999;
  printf("\nProcessing apertures from %s to %s...\n",use_range[0],use_range[1]);
  ...
  while (!stop)
  {
	...
	length=node_value("l");
	double_from_table(current_sequ->tw_table->name, "s", &tw_cnt[1], &s_end);
	...
	if ((pipelength = aper_external_file(pipefile, pipex, pipey)) > -1) ;
	else if (aper_bs(apertype, &ap1, &ap2, &ap3, &ap4, &pipelength, pipex, pipey))
	{
		...
		printf("\nNo apertype defined, or invalid aperture parameters.");
	...
	if (pipelength > -1)
	{
		...
		/* slice the node and make twiss for slices*/
		i=interp_node(&nint);
	  	i=embedded_twiss();
	
		for (jslice=0;jslice<=nint;jslice++)
	   	{
			minratio=ratio=999999;
			...
			for (angle=0;angle<twopi;angle+=dangle)
			{
				...
				/* send adjusted halo and its displacement to aperture calculation */
				aper_calc(deltax,deltay,&ratio,haloxsi,haloysi,halolength,
				              pipex,pipey,pipelength,minratio);
...
double aper_calc(double p, double q, double* minhl, double haloxcp[], double haloycp[], 
                     int halolength, double pipex[], double pipey[], int pipelength, double minratiocmp)
{
  int i=0, j=0, newhalolength=0, c=0, w=0, ver1, ver2;
  double halox[MAXARRAY], haloy[MAXARRAY], newhalox[MAXARRAY], newhaloy[MAXARRAY], dist_limit=0.0000000001;
  double a1, b1, a2, b2, xm, ym, h, l;
  FILE *pipe, *halo, *newhalo, *shifthalo, *ratiohalo;
  setbuf(stdout,(char *)NULL);  
  ...
  if (aper_chk_inside(p, q, pipex, pipey, dist_limit, pipelength))
  {
	...
	for (j=0;j<=halolength;j++)
	{
		...
		for (i=0;i<=pipelength;i++)
		{
			...
			ver2=aper_linepar(halox[j], haloy[j], halox[j+1], haloy[j+1], &a2, &b2);

			/*find meeting coordinates for infinitely long lines*/
			aper_intersect(a1, b1, a2, b2, pipex[i], pipey[i], 
			             halox[j], haloy[j], ver1, ver2, &xm, &ym);
			
			/*eliminate intersection points not between line limits*/
			if (-1 == aper_online(xm, ym, halox[j], haloy[j], halox[j+1], haloy[j+1], 
			                 dist_limit)) /*halo line*/
			{
	...
	for (i=0;i<=pipelength;i++)
	{
		for (j=0;j<=newhalolength;j++)
		{
			/*Find a and b parameters for line*/
			ver1=aper_linepar(p, q, newhalox[j], newhaloy[j], &a1, &b1);
			ver2=aper_linepar(pipex[i], pipey[i], pipex[i+1], pipey[i+1], &a2, &b2);

			/*find meeting coordinates for infinitely long lines*/
			aper_intersect(a1, b1, a2, b2, newhalox[j], newhaloy[j], 
			...
			{
				if (-1 != aper_online(p, q, newhalox[j], newhaloy[j], xm, ym, 
				                 dist_limit))  /*test line*/
				{
					h=sqrt((xm-p)*(xm-p)+(ym-q)*(ym-q));
					l=sqrt((newhalox[j]-p)*(newhalox[j]-p)
					         +(newhaloy[j]-q)*(newhaloy[j]-q));
...
/* *ap4 = half vertical axis ellipse                                    */
  ...
  if (!strcasecmp(apertype,"circle"))
  {
	*ap3=get_aperture(current_node, "var1"); /*radius circle*/
	...
	if ((*ap1) = (*ap2) = (*ap4) = (*ap3)) /* check if r = 0, skip calc if r = 0 */
	{
...
/* receives the name of file containing coordinates. Puts coordinates into tables. */
  ...
  if (file != NULL)
  {
	...
	{
		printf("Reading coordinates... %d: xcor = %lf and ycor = %lf\n", i, tablex[i], tabley[i]);
		i++;
		if (i >= MAXARRAY)
		{
     ...
     /* puts beam and aperture parameters at start of the aperture table */
{
  ...
  if (stmp = command_par_string("pipefile", this_cmd->clone)) h_length++;
  ...
  if (stmp = command_par_string("halofile", this_cmd->clone))
  {
  ...
  if (stmp = command_par_string("pipefile", this_cmd->clone))
  {
  ...
  sprintf(c_dummy, v_format("@ APERTURE LIMIT: %s, n1: %g, apertype: %s, aperture: %g - %g - %g - %g, 
                             tolerance: %g  - %g - %g"),lim->name,lim->n1,lim->apertype,
...
{
  double x, y, angle, alfa, theta, dangle, napex=19;
  int i=0;
...
void aper_trim_ws(char* string, int len)
{
  ...
  /* Replaces the first ws in a string with a '\0', thus translating */
  /* a FORTRAN-like string to C compatibility                        */
...
void pro_aperture(struct in_cmd* cmd)
{
  ...
  get_ex_range(range, current_sequ, use_range);
 
  /* make new aperture table */
  aperture_table=make_table(table, table, ap_table_cols, ap_table_types, current_sequ->n_nodes);
  aperture_table->dynamic=1; /* flag for table row access to current row */
 ...
 /* end of command decoding */
  ...
  tw_cnt[1]--; /* jumps back to "real" value */
  ...
  if (strcmp(aptwfile,"dummy"))
  {
  	out_table(tw_cp->name, tw_cp, aptwfile);
  }
     ...
     /* controls twiss module */
{
  ...
  */
  ...
  strcpy(aptwfile,filename); /* IW 05.11.2004 */
