...
def main():
    ...
    for fort_file in FortranSourceFiles:
        f = open(fort_file,'r')
        #if options.verbose:
        #    print("currently scanning file "+fort_file);
        ...
        for line in concatenatedLines:
            ...
            if state == lookForSubroutine:
                ...
                if m:
                    ...
                    res_index = subroutine(fort_file,"res_index","skew,mynorder,myn1,myn2,indexa,mynres",options)
    ...
    #                continue # next in the for loop
                ...
                if m:
                    trrun = subroutine(fort_file,"trrun","flag,turns,orbit0,oneturnmat,ibuf1,ibuf2, buf1, buf2,buf_dxt,buf_dyt,buf3,buf4,buf5,e_flag,ibuf3,buf6",options)
                    trrun.calledFromC()
                ...
                if m:             
                    ...
                    arguments = m.group(2)
                    newSubroutine = subroutine(fort_file,name,arguments,options) # BUG: arguments names are lowered
                    
                    for k,v in maybeFortranCalls.items(): # check if this Fortran subroutine is called from C   
                        if k == name:
                    ...
                    if m:
                        name = m.group(1)
                        arguments = ''
                        newSubroutine = subroutine(fort_file,name,arguments,options)
