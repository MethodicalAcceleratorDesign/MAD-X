...
module ptc_spin
  ...
  private GETMULB_TEAPOTR,GETMULB_TEAPOTP,GETMULB_TEAPOT
...
contains
  ...
  !    call PUSH_SPIN(c,DS,FAC,S,S%E_IJ,before,k,POS)
  ...
  SUBROUTINE stroboscopic_average(ring,xs0,xst,x0,pos,mstate0,nturn,kp,n)
    ...
    do k=1,nturn
       ...
       if(mod(k,kp)==0) then  ! kp
          write(6,*) k,"$$$$$$$$$$$$$$$$$$$$$$$$$"
          do i=1,3
             norm=sqrt(xst%s(1)%x(i)**2+xst%s(2)%x(i)**2+xst%s(3)%x(i)**2)
             if(norm>=zero) then
                norm=1.d0/norm
             endif
             write(6,*) 1.d0/norm
